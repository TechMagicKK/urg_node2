...
def generate_launch_description():
    ...
    config_file_path_2nd = os.path.join(
        get_package_share_directory('urg_node2'),
        'config',
        'params_ether_right.yaml'
    )

    # パラメータファイルのパス設定（2台目）
    config_file_path_3rd = os.path.join(
        get_package_share_directory('urg_node2'),
        'config',
        'params_ether_center.yaml'
    )
    ...
    # パラメータファイルのロード（2台目）
    with open(config_file_path_3rd, 'r') as file:
        config_params_3rd = yaml.safe_load(file)['urg_node2']['ros__parameters']
    ...
    # urg_node2をライフサイクルノードとして起動（2台目）
    lifecycle_node_3rd = LifecycleNode(
        package='urg_node2',
        executable='urg_node2_node',
        name=LaunchConfiguration('node_name_3rd'),
        remappings=[('scan', LaunchConfiguration('scan_topic_name_3rd'))],
        parameters=[config_params_3rd],
        namespace='',
        output='screen',
    )
    ...
    # Unconfigure状態からInactive状態への遷移（auto_startがtrueのとき実施）（2台目）
    urg_node2_node_3rd_configure_event_handler = RegisterEventHandler(
        event_handler=OnProcessStart(
            target_action=lifecycle_node_3rd,
            on_start=[
                EmitEvent(
                    event=ChangeState(
                        lifecycle_node_matcher=matches_action(lifecycle_node_3rd),
                        transition_id=Transition.TRANSITION_CONFIGURE,
                    ),
                ),
            ],
    ...
    # Inactive状態からActive状態への遷移（auto_startがtrueのとき実施）（2台目）
    urg_node2_node_3rd_activate_event_handler = RegisterEventHandler(
        event_handler=OnStateTransition(
            target_lifecycle_node=lifecycle_node_3rd,
            start_state='configuring',
            goal_state='inactive',
            entities=[
                EmitEvent(
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('node_name_3rd', default_value='urg_node2_3rd'),

        DeclareLaunchArgument('scan_topic_name_1st', default_value='scan_left'),
        DeclareLaunchArgument('scan_topic_name_2nd', default_value='scan_right'),
        DeclareLaunchArgument('scan_topic_name_3rd', default_value='scan_center'),
        lifecycle_node_1st,
        lifecycle_node_2nd,
        lifecycle_node_3rd,
        ...
        urg_node2_node_3rd_configure_event_handler,
        urg_node2_node_3rd_activate_event_handler,        
    ])
